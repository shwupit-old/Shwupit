...
export default function Uploader({
  ...
  const uploadToSupabase = async (files: File[]) => {
    ...
    for (const file of files) {
      ...
      if (data) {
        ...
        uploadedFiles.push({ id: data.path, original: publicURL });
      }
    }
    return uploadedFiles;
  };

  const { mutate, isLoading } = useMutation(uploadToSupabase, {
    onSuccess: (response) => {
  ...
  return (
    <div className="flex flex-wrap gap-2.5">
      <div
        {...getRootProps({
          className: 'relative border-dashed border-2 border-light-500 dark:border-dark-600 text-center flex flex-col justify-center hover:text-black dark:hover:text-light items-center cursor-pointer focus:border-accent-400 focus:outline-none',
        })}
      >
        ...
        attachments.map(({ id, original }) => (
          <div key={id} className="group relative h-20 w-20 overflow-hidden rounded-md">
            <div className="relative h-full w-full overflow-hidden rounded-md">
              <Image alt="Attachment" src={original} fill className="object-cover" />
            </div>
